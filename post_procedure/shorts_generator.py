...
TEMPLATES = {
    ...
    # Раньше: «делаешь {proc} впервые» → подставлялся родительный из PROC_NAMES
    # → «делаешь лазерной шлифовки впервые». Переформулировано: «впервые после X»
    # сидит с родительным естественно для всех 13 процедур.
    "first_time": "что ждёт впервые после {proc} — день за днём",
    ...
    # Maintenance family — долгосрочный уход «после периода восстановления»:
    # что делать через недели/месяцы чтобы эффект {proc} держался дольше.
    "longterm_habits":     "три привычки которые укорачивают эффект {proc} в 2 раза — у всех они есть",
    "longterm_ingredients":"три ингредиента в твоём уходе которые продлевают эффект {proc} в разы",
    "longterm_spf":        "SPF после {proc} — как наносить чтобы результат держался не месяц, а год",
}
...
3. block3 — punchline: самый сильный/неочевидный пункт. В конце — КОРОТКИЙ like-bait одной фразой:
   «двойной тап если делала так», «лайк если узнала себя», «двойной тап если откликнулось». Like-bait
   работает лучше вопроса: для shorts-алгоритма like-rate важнее комментариев.
4. CTA — не «подписывайся», не вопрос. Короткая фраза-передача ценности: «Полный список — в Telegram @posleprocedur» или «Забирай схему по дням в Telegram @posleprocedur».
...
Верни ТОЛЬКО валидный JSON без пояснений:
{
  ...
  "block3": "1 предложение — самый сильный пункт + короткий like-bait в конце ('двойной тап если...', 'лайк если...')",
...
3. block3 — punchline: самый сильный/неочевидный пункт. В конце — КОРОТКИЙ like-bait одной фразой:
   «двойной тап если откликнулось», «лайк если узнала себя», «двойной тап если делала так», «лайк если такое было».
   Like-bait работает лучше вопроса (вопрос провоцирует мысль, лайк — действие пальцем). Для shorts-алгоритма
   like-rate — главный быстрый позитивный сигнал, важнее комментариев.
4. CTA — короткая фраза-передача ценности: «Полный список — в Telegram @posleprocedur» или «Забирай схему по дням в Telegram @posleprocedur». Не «подписывайся», не вопрос. Ритм не должен провисать.
...
MAINTENANCE_SCRIPT_SYSTEM = """Ты пишешь сценарии для 30-секундных Shorts про долгосрочный уход — что делать ПОСЛЕ периода острого восстановления, чтобы эффект {proc} держался дольше. Аудитория — женщины у которых уже прошло 2+ недели после процедуры, кожа зажила, но результат начинает таять.

Голос: разговорный, «ты», уверенный. Говоришь как косметолог, который не торопится и объясняет «почему».

КЛЮЧЕВОЕ ОТЛИЧИЕ от recovery-сценариев: НИКАКИХ «первых дней», «не мочить», «пантенол». Только долгосрочные факторы: привычки быта, активы в ежедневном уходе, защита (SPF, антиоксиданты). Зритель уже не боится — он хочет «как сохранить то что есть».

ВАЖНО — структура удержания:
1. Хук — curiosity_gap + явное обещание "3 [привычки/ингредиента/правила]" БЕЗ раскрытия ответа. Подчеркни что эффект процедуры тает быстрее чем должен, и это решаемо.
   Примеры: «3 привычки которые укорачивают эффект {proc} в 2 раза — у всех они есть, и почти никто не замечает»
            «3 ингредиента которые в разы продлевают результат {proc} — но в кремах их редко ищут»
2. block1/2/3 — три конкретных пункта с микро-объяснением «почему» (это отличает maintenance от recovery — там работало просто «делай/не делай», здесь зритель уже опытнее). На экране метки 1/3, 2/3, 3/3.
3. block3 — самый сильный/контринтуитивный пункт. В конце — КОРОТКИЙ like-bait одной фразой:
   «двойной тап если так делаешь», «лайк если узнала себя», «двойной тап если это про тебя». Like-bait
   работает лучше вопроса: для shorts-алгоритма like-rate важнее комментариев.
4. CTA — не вопрос, не «подписывайся». Только короткая передача КОНКРЕТНОГО магнита: «Полный список ингредиентов — в Telegram @posleprocedur» или «Список привычек одним сообщением — в закрепе @posleprocedur».

ОСТОРОЖНО с ингредиентами: называй классы (пептиды-миорелаксанты, церамиды, ниацинамид, антиоксиданты, ретинол низкой концентрации) — НЕ конкретные торговые названия. Не давай медицинских утверждений категорически («лечит», «омолаживает на N лет») — формулируй мягко («поддерживает», «помогает дольше держать»). Если не уверен в безопасности рекомендации для {proc} — выбирай более общий ингредиент.

Верни ТОЛЬКО валидный JSON без пояснений:
{
  "hook":   "1 предложение — curiosity_gap + обещание '3 [чего-то]' без раскрытия, упоминая что эффект тает",
  "block1": "1 предложение — первый пункт + микро-объяснение почему это работает/мешает",
  "block2": "1 предложение — второй пункт + микро-объяснение",
  "block3": "1 предложение — самый сильный/контринтуитивный пункт + короткий like-bait ('двойной тап если...', 'лайк если...')",
  "cta":    "1 фраза — короткая передача конкретного магнита с ссылкой на @posleprocedur"
}

Суммарно 50-65 слов — это плотнее обычного recovery-сценария, потому что нужно объяснить «почему». Никакого текста кроме JSON."""

# Шаблоны которые используют MAINTENANCE_SCRIPT_SYSTEM вместо обычного SCRIPT_SYSTEM
MAINTENANCE_TEMPLATES = {"longterm_habits", "longterm_ingredients", "longterm_spf"}
...
HOOK_STYLE = {
    ...
    "first_time": "Hook (curiosity_gap): первый раз после процедуры открывает то что обычно скрывают. Пример: «Впервые после {proc}? Есть кое-что что тебе не скажут в кабинете». NB: пиши «впервые после {proc}» или «первый раз после {proc}» — НИКОГДА не пиши «делаешь {proc} впервые» (падежный косяк).",
    ...
    # Maintenance hooks — упор на «уже не острый период, но эффект тает». Ловим тех кому 2 недели+ после {proc}.
    "longterm_habits":      "Hook (curiosity_gap): «3 ежедневные привычки которые сокращают эффект {proc} в 2 раза — почти все так делают и не подозревают». Блоки: бытовые действия (сон на лице, мимика, активный спорт, горячий душ и т.д.) с конкретикой почему это бьёт по результату.",
    "longterm_ingredients": "Hook (curiosity_gap): «3 ингредиента в твоей косметичке которые продлевают эффект {proc} — но почти никто не использует все три». Блоки: каждый — конкретный класс активов (пептиды, церамиды, ниацинамид, антиоксиданты и т.д.), что делает и когда применять.",
    "longterm_spf":         "Hook (warning): «Половина результата {proc} уходит из-за неправильного SPF — и дело не в цифре на упаковке». Блоки: 1) какой SPF и сколько грамм на лицо реально нужно, 2) когда обновлять, 3) почему ZNO/химия работают по-разному (без вкусовщины).",
}
...
KICKER = {
    ...
    "norm_alarm":  "НОРМА?",
    "longterm_habits":      "ПРИВЫЧКА.",
    "longterm_ingredients": "ПРОДЛИ.",
    "longterm_spf":         "SPF.",
    # general
    "money":       "ВПУСТУЮ.",
    "sun":         "СОЛНЦЕ.",
...
def _generate_script(procedure: str, template: str) -> dict:
    if procedure == "general":
        ...
        elif template in MAINTENANCE_TEMPLATES:
            system = MAINTENANCE_SCRIPT_SYSTEM.replace("{proc}", proc_nom)
        else:
